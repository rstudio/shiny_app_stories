...
ui <- fluidPage(
  ...
  div(id = "page-top",
      radioButtons("current_theme", "App Theme:",
                   c("Light" = "light", "Dark" = "dark"), inline = TRUE),
  ...
  div(id = "data_info", class = "small_text",
...
server <- function(input, output, session) {
  ...
  output$weather_plot <- renderPlot({
    ...
    bindCache(input$city, get_md(), getCurrentOutputInfo()$fg(), sizePolicy = sizeGrowthRatio(width = 400, height = 600))
  ...
  output$todays_weather <- renderUI({
    ...
    todays_weather <- city_data()$temperature %>%
      filter(get_md(date) == get_md())

    div(
      icon("calendar-day"), "Today's Weather:",
      get_md(), "in ", strong(input$city), "is normally ",
      HTML(print_deg(todays_weather$avg, add_F = TRUE)), "with a high of", HTML(print_deg(todays_weather$max)),
